...
const CreateInvoiceScreen = () => {
  ...
  const handleQRScan = (scannedCode: string) => {
    console.log('📱 [Invoice] Scanned QR Code:', scannedCode);
    ...
    try {
      ...
      if (numberMatch && numberMatch[1]) {
        ...
        console.log('✅ [Invoice] Extracted SKU from text:', skuToSearch);
      }
    }

    // Try to find existing part by SKU
    const existingPart = getPartBySku(skuToSearch);
    console.log('🔍 [Invoice] Looking for part with SKU:', skuToSearch, 'Found:', !!existingPart);
